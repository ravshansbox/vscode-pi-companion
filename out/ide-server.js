...
class IDEServer {
    constructor(log, _outputChannel) {
        this.port = 0;
        this.authToken = '';
    ...
    async start(context) {
        ...
        this.authToken = (0, crypto_1.randomUUID)();
        // Create MCP server
        this.mcpServer = this.createMcpServer();
        // Create OpenFilesManager
        this.openFilesManager = new open_files_manager_1.OpenFilesManager(context);
    ...
    createMcpServer() {
        ...
        server.tool('get_open_files', 'Get all open files with their content, cursor position, and selection', {}, async () => {
            const context = this.openFilesManager?.getState();
            return {
                content: [
                    {
        ...
        // Tool: read_file
        server.tool('read_file', 'Read the contents of a file', { path: { type: 'string', description: 'Absolute path to the file' } }, async (args) => {
            try {
        ...
        // Tool: goto
        server.tool('goto', 'Open a file at a specific location', {
            ...
            try {
                ...
                if (args.line !== undefined) {
                    const position = new vscode.Position(Math.max(0, args.line - 1), Math.max(0, (args.character || 1) - 1));
    ...
    broadcastIdeContextUpdate() {
        ...
        const context = this.openFilesManager.getState();
        // Send to SSE clients
        const sseClients = this._sseClients;
        if (sseClients) {
